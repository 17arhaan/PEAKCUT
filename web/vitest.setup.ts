...
// container (`peakcut_test`) — created via docker init scripts or
// this fallback guard.
process.env.DATABASE_URL ??=
  "postgresql://postgres:postgres@localhost:5432/peakcut_test";
...
try {
  execSync(
    `docker exec web-db-1 psql -U postgres -c "CREATE DATABASE peakcut_test" 2>&1 || true`,
    { stdio: "ignore" }
  );
